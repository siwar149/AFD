...
species <- score_species %>%
  group_by(taxonid) %>%
  summarise(score=sum(score_sum)) %>%
  filter(score>0)

length(species$taxonid) # 4,268 species analysed 

#species_info <- readRDS("/Red_list_species.rds")

species_info <- s3read_using(FUN = readRDS,
...
ggplot(species) +
  geom_density(aes(x=score,fill=class_name)) +
  facet_wrap(vars(class_name),nrow=3) +
  theme_ridges() +
...
country_score <- score_species %>%
  group_by(country) %>%
  summarize(score=sum(score_sum)) %>%
  distinct() %>%
  left_join(match_pays_gloria_WM, by=c("country"="pays_g"))

map <- world_map %>%
  left_join(country_score,by=c("region"="WM"))
...
species <- species %>%
  mutate(score_scaled = (score - min(score)) / (max(score) - min(score))) # min / max normalization de toute la colonne score